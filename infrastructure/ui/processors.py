...
- Write .env file from API credentials
- Test and save WordPress connection
...
def process_api_credentials(response_path: str) -> Dict[str, Any]:
    """Process API credentials form and update .env file."""
    data = _read_response(response_path)
    if not data:
        return {"status": "error", "error": "No response data"}
    ...
    env_lines = ["# AgenticMarketingPro — API Credentials", "# Generated by form processor", f"# Last updated: {datetime.now().isoformat()}", ""]
    ...
    logger.info(f"Updated .env with {updated_count} credentials")
    return {
        "status": "success",
        "credentials_updated": updated_count,
    ...
    try:
        ...
        if result.get("status") == "connected":
            ...
            env_lines = [
                ...
                f"WP_USERNAME={username}",
                f"WP_APP_PASSWORD={password}",
                f"WP_SEO_PLUGIN={data.get('wp_seo_plugin', 'None')}",
                f"WP_POST_TYPE={data.get('wp_post_type', 'post')}",
                f"WP_AUTHOR_ID={data.get('wp_author_id', '1')}",
            ]
            env_path = Path(".env")
            with open(env_path, "a", encoding="utf-8") as f:
                f.write("\n".join(env_lines) + "\n")

            logger.info(f"WordPress connected: {site_url} (user: {result.get('username')})")
            return {
                "status": "connected",
                "site_url": site_url,
