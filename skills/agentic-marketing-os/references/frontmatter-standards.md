...
Every note in the vault must have valid YAML frontmatter. QA rejects missing/invalid fields.

## Required fields (all notes)
- `type:` — note type (see registry below)
- `last_updated:` — YYYY-MM-DD format
- `tags:` — list of approved tags
...
- Tags must exist in `tag-dictionary.md` (in vault: `11-Ops/playbooks/tag-dictionary.md`)
- QA rejects notes with undefined tags (auto-fix: suggest closest match)
...
- `#type/[type-name]` — Content type

## Validation
- Pre-commit hook runs YAML linter on every commit
- QA validates frontmatter on every artifact
- Playbook Librarian runs monthly audit for missing fields
